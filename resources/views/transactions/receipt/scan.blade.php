        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Dispatch Storage Scan</h3>
                </div>
                <div class="card-body">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-8">
                            <input type="text" id="barcode" name="barcode" class="form-control form-control-sm" oninput="receiptStorageScan()" required>
                        </div>
                    </div>
                </div>
...
<script>
            ...
            success: function (response) {
                ...
                if(response){
                    ...
                    if(response.scan_complete){
                        sweetAlertMessage('success', 'Success', 'Receipt Scan Completed!', false, "{{ route('receipt-scan.index') }}");
                    }
                }
                barcode.value = '';
            }
    ...
    function fetchScanDetails(){
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('ajax.fetch-receipt-scan-details') }}",
            data: {
                dispatch_id: {{$id}},
            },
            dataType: "json",
