...
<div class="card mt-4">
    <div class="card-body">
        <h6 class="card-title">Opening Stock Report</h6>

        <form action="{{ route('opening-stock-report.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6 form-group">
                    <div class="col-md-12 form-group">
                        <label for="opening_number">Opening Number</label>
                        <select name="opening_number" id="opening_number" class="js-example-basic-single form-control">
                            <option value="">--select--</option>
                            @foreach($openings as $opening)
                            <option value="{{ $opening->opening_number }}">{{ $opening->opening_number }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
